<?php
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['action']) && $_GET['action'] == 'tambah-menu') {
    ...
    if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] == 0) {
        $target_dir = "uploads/menu/"; // Direktori untuk menyimpan file yang diunggah
        ...
        if ($check !== false) {
            ...
            if ($_FILES["gambar"]["size"] <= 5000000) {
                ...
                if ($imageFileType == "jpg" || $imageFileType == "jpeg" || $imageFileType == "png") {
                    ...
                    $sql = "INSERT INTO cms_menu (nama_menu, detail, harga) VALUES (?, ?, ?)";
                    $stmt = $koneksi->prepare($sql);
                    $stmt->bind_param("sss", $nama_menu, $detail, $harga);
                    ...
                    if ($stmt->execute()) {
                        ...
                        if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
                            ...
                            $update_sql = "UPDATE cms_menu SET gambar = ? WHERE id = ?";
                            ...
                            if ($update_stmt->execute()) {
                                ...
                                header("Location: page.php?page=menu"); // Ganti dengan halaman yang Anda inginkan
